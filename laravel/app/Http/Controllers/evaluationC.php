        ...
        if($starValue!=NULL){
            ...
            for($i=0;$i<=$count-1;$i++) {
                ...
                DB::table('menu_evaluation')->insert(array(
                    array('name' => $orderName, 'rest_name' => $openRestName, 'kind' => $orderKind, 'm_star' => $starValue[$i], 'date' => $date)//新增顧客評價
                ));
            ...
            DB::table('rest_evaluation')->insert(array(
                array('name' => $orderName, 'rest_Name' => $openRestName, 'r_star' => $starValue[$count], 'date' => $date)//新增至資料庫
            ));
