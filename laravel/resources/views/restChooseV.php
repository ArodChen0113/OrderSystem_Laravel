...
<form name="rest_open" action="controller/restaurant_controller.php" method="post" enctype="multipart/form-data">
    <table border="1">
        ...
        <tr>
            <td>請選擇今日開餐餐廳：</td>
            <td><select style="width:240px" name="kind1" onchange="window.location='action_rKControl1?action=control1&select1='+this.value">
                    <?php
            ...
            <td><select style="width:240px" name="kind2" onchange="window.location='controller/rest_kind_controller.php?action=control2&select1=<?php ?>//&select2='+this.value">
                    <?php
                        if($_SESSION['rest_name']!=NULL){
                            $restName=$_SESSION['rest_name'];
                            $num=count($restName);
                            for ($k=0;$k<=$num;$k++) {
                                $v2 = $restName[$k]; ?>　
                                <option value="<?php echo $v2->rest_name; ?>"><?php echo $v2->rest_name; ?></option>
                                <?php
                            }}else{
                            ?>
                    <option value="">
    ...
    <input type="hidden" name="open_restName" value="<?php ?>">
